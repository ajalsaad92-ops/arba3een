...
const borderIcon = L.divIcon({ className:'border-marker', html:`<div style="width:20px;height:20px;transform:rotate(45deg);background:#10B981;border:2px solid #0d0d0d;box-shadow:0 2px 4px rgba(0,0,0,.4)"></div>`, iconSize:[20,20], iconAnchor:[10,10]});
...
const procIcon = L.divIcon({ className:'proc-marker', html:`<div style="width:12px;height:12px;background:#F59E0B;border:2px solid #0d0d0d;transform:rotate(45deg)"></div>`, iconSize:[14,14], iconAnchor:[7,7]});
      ...
      <MapContainer center={MAP_CONFIG.center} zoom={MAP_CONFIG.zoom} minZoom={MAP_CONFIG.minZoom} maxZoom={MAP_CONFIG.maxZoom}
        ...
        {((iraqAdm1 as any).features as any[]).map((feat) => {
          ...
          return <Marker key={`lbl-${code}`} position={[lat,lng]} interactive={false}
            icon={L.divIcon({ className:'gov-label', html:`<div style="color:#232323;font-family:Cairo;font-size:10px;font-weight:700;text-shadow:0 0 3px #fff,0 0 3px #fff;white-space:nowrap;pointer-events:none">${NAMES_AR[code]||code}</div>`, iconSize:[80,14], iconAnchor:[40,7] })} />;
        })}
