    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        {grouped.map(({ group, fields }) => {
          ...
          return (
            <div key={group.id} className="bg-[#1a1a1a] border border-[#232323] rounded-xl overflow-hidden">
              <div className="p-3 flex items-center gap-2">
                ...
                <button onClick={() => { setEditingGroup(group); setCreatingGroup(false); }} className="p-1.5 rounded hover:bg-[#232323] text-slate-400" title="تعديل المجموعة">
              ...
              {expanded && (
                <div className="border-t border-[#232323] divide-y divide-[#232323]">
                  {fields.map(f => {
                    const Tm = TYPE_META[f.fieldType] || TYPE_META.text;
                    const TIcon = Tm.icon;
                    return (
                      <div key={f.id} className={`p-3 flex items-center gap-3 ${f.isHidden ? 'opacity-50' : ''}`}>
                        <div className="w-8 h-8 rounded-md bg-[#0d0d0d] border border-[#232323] flex items-center justify-center text-slate-400">
                        ...
                        <button onClick={() => toggleFieldHidden(f)} className="p-1.5 rounded hover:bg-[#232323] text-slate-400" title={f.isHidden ? 'إظهار' : 'إخفاء'}>
                          {f.isHidden ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                        </button>
                        <button onClick={() => { setEditingField(f); setCreatingInGroup(null); }} className="p-1.5 rounded hover:bg-[#232323] text-slate-300" title="تعديل">
                  ...
                  <div className="p-2">
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-1.5 py-2 rounded-lg bg-[#0d0d0d] border border-dashed border-[#232323] hover:border-amber-500/40 text-amber-400 text-xs font-bold"
                    >
...
function GroupEditor({ initial, isNew, onCancel, onSaved }: any) {
  ...
  return (
    <Modal title={isNew ? 'مجموعة جديدة' : `تعديل المجموعة: ${initial.titleAr}`} onClose={onCancel}>
      ...
      <div className="flex gap-2 pt-3 mt-3 border-t border-[#232323]">
        <button onClick={onCancel} className="flex-1 py-2 rounded-lg bg-[#232323] hover:bg-[#2c2c2c] text-slate-300 text-sm font-bold">إلغاء</button>
...
function FieldEditor({ initial, groupId, users, onCancel, onSaved }: any) {
  ...
  return (
    <Modal title={isNew ? 'حقل جديد' : `تعديل الحقل: ${initial.labelAr}`} onClose={onCancel}>
            ...
            return (
              <button
                ...
                className={`p-2 rounded-md text-[11px] flex flex-col items-center gap-1 border transition-colors ${
                  ...
                  'border-[#232323] bg-[#0d0d0d] text-slate-400 hover:border-[#2c2c2c]'
                }`}
              >
      ...
      {fieldType === 'select' && (
        <div className="mt-2 rounded-lg bg-[#0d0d0d] border border-[#232323] p-3 space-y-2">
      ...
      <FieldRow label={`تخصيص الحقل لمستخدمين محدّدين (${allowedUserIds.length === 0 ? 'الافتراضي: للكل' : allowedUserIds.length + ' مستخدم'})`}>
        <div className="bg-[#0d0d0d] border border-[#232323] rounded-md p-2 max-h-44 overflow-y-auto grid grid-cols-1 gap-1">
          {users.map((u: any) => (
            <label key={u.id} className="flex items-center gap-2 text-xs text-slate-300 cursor-pointer hover:bg-[#232323]/40 px-1 py-0.5 rounded">
...
// ─── helpers ─────────────────────────────────────────────────────────
const inputCls = 'w-full bg-[#232323] border border-[#2c2c2c] rounded-md px-3 py-2 text-sm text-white placeholder-slate-500 focus:border-amber-500/40 focus:outline-none';
...
function Modal({ title, onClose, children }: any) {
  return (
    <div className="fixed inset-0 z-[700] bg-black/70 flex items-center justify-center p-3 animate-fade-in-up" onClick={onClose}>
      <div onClick={e => e.stopPropagation()} className="w-full max-w-xl max-h-[90vh] overflow-y-auto bg-[#0d0d0d] border border-amber-500/30 rounded-2xl shadow-2xl">
        <div className="p-4 border-b border-[#232323] flex items-center justify-between sticky top-0 bg-[#0d0d0d] z-10">
          <div className="font-display font-black text-amber-400 text-sm">{title}</div>
          <button onClick={onClose} className="w-8 h-8 rounded-lg bg-[#232323] hover:bg-[#2c2c2c] flex items-center justify-center text-slate-400">
            <X className="w-4 h-4" />
          </button>
        </div>
