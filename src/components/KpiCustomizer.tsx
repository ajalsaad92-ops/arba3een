...
export default function KpiCustomizer() {
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen(o => !o)}
        className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-[#1a1a1a] border border-[#232323] text-[11px] text-slate-300 hover:border-amber-500/30 transition-colors"
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 mt-2 w-64 bg-[#1a1a1a] border border-[#232323] rounded-xl shadow-2xl z-[801] overflow-hidden">
              ...
              {catalog.map(k => {
                ...
                return (
                  <button
                    key={k.id}
                    onClick={() => toggle(k.id)}
                    className={`w-full flex items-center gap-2 px-2 py-2 rounded-md text-xs text-right hover:bg-[#232323]/60 ${on ? 'text-amber-300' : 'text-slate-400'}`}
                  >
