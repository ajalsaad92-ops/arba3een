    ...
    html: `
      <div style="position:relative;width:32px;height:32px;transform:translate(-50%,-100%);">
        <svg width="32" height="40" viewBox="0 0 32 40">
          <path d="M16 0 C7 0 0 7 0 16 C0 28 16 40 16 40 C16 40 32 28 32 16 C32 7 25 0 16 0 Z" fill="${color}" stroke="#0d0d0d" stroke-width="1.5"/>
          <circle cx="16" cy="15" r="6" fill="#0d0d0d"/>
...
export default function MapPicker({
  ...
  return (
    <div className="fixed inset-0 z-[700] bg-black/70 flex items-center justify-center p-3 animate-fade-in-up" onClick={onCancel}>
      <div onClick={(e) => e.stopPropagation()} className="w-full max-w-3xl max-h-[95vh] flex flex-col bg-[#0d0d0d] border border-amber-500/30 rounded-2xl overflow-hidden shadow-2xl">
        <div className="p-3 sm:p-4 border-b border-[#232323] flex items-center justify-between gap-2">
          <div>
          ...
          <button onClick={onCancel} className="w-9 h-9 rounded-lg bg-[#232323] hover:bg-[#2c2c2c] flex items-center justify-center text-slate-400 shrink-0">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="relative flex-1 min-h-[55vh] sm:min-h-[420px]">
          ...
          <div className="absolute top-2 left-2 bg-[#0d0d0d]/85 border border-[#232323] rounded-md px-2.5 py-1 text-[10px] sm:text-[11px] text-amber-300 pointer-events-none flex items-center gap-1.5">
          ...
          <button
            onClick={locateMe}
            disabled={locating}
            className="absolute top-2 right-2 z-[500] flex items-center gap-1.5 bg-[#0d0d0d]/90 border border-blue-500/40 hover:bg-blue-500/20 rounded-md px-2.5 py-1.5 text-[10px] sm:text-[11px] text-blue-300 font-bold disabled:opacity-60"
          >
            {locating ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Crosshair className="w-3.5 h-3.5" />}
            موقعي
          </button>
          {livePos && (
            <div className="absolute bottom-2 left-2 bg-[#0d0d0d]/85 border border-blue-500/30 rounded-md px-2.5 py-1 text-[10px] sm:text-[11px] text-blue-300 pointer-events-none flex items-center gap-1.5">
              <Crosshair className="w-3.5 h-3.5" /> موقعك الحالي
            </div>
          )}
        </div>

        <div className="p-3 sm:p-4 border-t border-[#232323] flex items-center justify-between gap-2 flex-wrap">
          <div className="flex gap-2">
            {mode !== 'single' && pts.length > 0 && (
              <>
                <button
                  onClick={() => { setPts(p => p.slice(0, -1)); setSnapped(null); }}
                  className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-[#232323] hover:bg-[#2c2c2c] text-slate-300 text-xs font-bold"
                >
          ...
          <div className="flex gap-2">
            <button onClick={onCancel} className="px-4 py-2 rounded-lg bg-[#232323] hover:bg-[#2c2c2c] text-slate-300 text-sm font-bold">إلغاء</button>
            {mode === 'single' ? (
              <button
                disabled={!single}
