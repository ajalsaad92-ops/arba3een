...
function reportToRow(rep: DailyReport): any {
  // NOT-NULL columns must never receive null/undefined. Numeric counters
  // coalesce to 0, text columns to '' so partial/dynamic forms never violate
  // the schema's not-null constraints.
  const num = (v: any) => (typeof v === 'number' && !Number.isNaN(v) ? v : 0);
  const txt = (v: any) => (v == null ? '' : v);
  return {
    ...
    deployment_locations: txt(rep.deploymentLocations),
    deployment_formations: txt(rep.deploymentFormations),
    coordination_sectors: txt(rep.coordinationSectors),
    coordination_joint_ops: txt(rep.coordinationJointOps),
    incidents_count: num(rep.incidentsCount),
    incidents_details: txt(rep.incidentsDetails),
    violations_count: num(rep.violationsCount),
    violations_area: txt(rep.violationsArea),
    violations_time_detail: txt(rep.violationsTimeDetail),
    violations_details: txt(rep.violationsDetails),
    deaths_count: num(rep.deathsCount),
    deaths_location_mgrs: txt(rep.deathsLocationMgrs),
    ...
    events_coordinates: Array.isArray(rep.eventsCoordinates) ? rep.eventsCoordinates : [],
    ...
    visitors_routes: txt(rep.visitorsRoutes),
    ...
    procession_waypoints: Array.isArray(rep.processionWaypoints) ? rep.processionWaypoints : [],
    ...
    extra_fields: rep.extraFields && typeof rep.extraFields === 'object' ? rep.extraFields : {},
  };
}
