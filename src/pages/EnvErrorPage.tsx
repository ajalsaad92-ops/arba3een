...
VITE_SUPABASE_PUBLISHABLE_KEY=sb_publishable_xxxxxxxxxxxxxxxxxxxx`;
  ...
  return (
    <div className="min-h-screen w-screen flex items-center justify-center p-4 bg-[#0d0d0d] grid-pattern">
      <div className="absolute top-1/4 right-1/4 w-[500px] h-[500px] rounded-full bg-red-500/10 blur-3xl pointer-events-none" />
      <div className="relative z-10 w-full max-w-2xl bg-gradient-to-br from-[#1a1a1a] to-[#0d0d0d] border border-red-500/30 rounded-2xl p-8 shadow-2xl shadow-black/60">
        ...
        <div className="mb-5">
          ...
          <ul className="space-y-2">
            {REQUIRED_VARS.map(v => (
              <li key={v.key} className="flex items-start gap-2 p-2.5 rounded-lg bg-[#0d0d0d] border border-[#232323]">
        ...
        <div className="mb-5">
          ...
          <div className="relative">
            <pre dir="ltr" className="text-[11px] font-mono text-emerald-300 bg-[#0d0d0d] border border-[#232323] rounded-lg p-3 overflow-x-auto whitespace-pre">
{exampleEnv}
            </pre>
            <button
              onClick={copyExample}
              className="absolute top-2 left-2 p-1.5 rounded-md bg-[#232323] hover:bg-[#2c2c2c] text-slate-400 hover:text-white transition-colors"
              title="نسخ المثال"
            >
