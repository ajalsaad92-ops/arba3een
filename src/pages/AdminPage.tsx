    ...
    <div className="h-full overflow-y-auto bg-[#0d0d0d] p-3 md:p-5" dir="rtl">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
          <div className="lg:col-span-2 bg-[#1a1a1a] border border-[#232323] rounded-xl overflow-hidden">
            ...
            <div className="divide-y divide-[#232323] max-h-[45vh] lg:max-h-[600px] overflow-y-auto">
              ...
              {paginated.map(u => {
                ...
                return (
                  <div key={u.id} className={`p-3 hover:bg-[#232323]/40 cursor-pointer ${editing?.id===u.id ? 'bg-amber-500/10 border-r-2 border-amber-500':''}`} onClick={()=>startEdit(u)}>
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      {loc ? (
                        ...
                        <span className="text-[10px] px-1.5 py-0.5 rounded border bg-[#0d0d0d] text-slate-500 border-[#232323] flex items-center gap-1"><MapPin className="w-3 h-3" /> لا يوجد موقع</span>
                      )}
                    </div>
                  </div>
                );
            ...
            {totalPages > 1 && (
              <div className="p-2 border-t border-[#232323] flex items-center justify-between text-[11px] text-slate-400">
                <span>صفحة {page} / {totalPages}</span>
                <div className="flex gap-1">
                  <button disabled={page<=1} onClick={()=>setPage(p=>p-1)} className="px-2 py-1 rounded bg-[#232323] disabled:opacity-30">السابق</button>
                  <button disabled={page>=totalPages} onClick={()=>setPage(p=>p+1)} className="px-2 py-1 rounded bg-[#232323] disabled:opacity-30">التالي</button>
                </div>
              </div>
            )}
          </div>
            ...
            {!creating && !editing ? (
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button onClick={()=>{ setCreating(false); setEditing(null); setDraft({}); }} className="p-1 rounded hover:bg-[#232323]"><X className="w-4 h-4 text-slate-400" /></button>
                </div>
                ...
                <FormField label={creating ? 'اسم المستخدم (للدخول)' : 'تغيير اسم المستخدم (اختياري)'} hint={username ? `سيُسجّل الدخول: ${username.toLowerCase().replace(/[^a-z0-9._-]+/g,'')||'username'}@ops.iq` : undefined} error={usernameError} id="adm-user">
                  <input id="adm-user" value={username} onChange={e=>{ setUsername(e.target.value); setUsernameError(''); }}
                    ...
                    className="w-full bg-[#232323] border border-[#2c2c2c] rounded-md px-3 py-2 text-sm text-white text-left focus:border-amber-500/40 focus:outline-none" />
                </FormField>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <FormField label="الدور" required id="adm-role">
                    <select id="adm-role" value={draft.role ?? 'agent'} onChange={e=>setDraft(d=>({...d, role: e.target.value as Role}))}
                      className="w-full bg-[#232323] border border-[#2c2c2c] rounded-md px-3 py-2 text-sm text-white">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <FormField label={creating ? 'كلمة المرور' : 'كلمة مرور جديدة'} required={creating} id="adm-pass"
                    ...
                    <input id="adm-pass" type="password" value={draft.password ?? ''} onChange={e=>setDraft(d=>({...d, password: e.target.value}))}
                      className="w-full bg-[#232323] border border-[#2c2c2c] rounded-md px-3 py-2 text-sm text-white focus:border-amber-500/40 focus:outline-none" dir="ltr" />
                    {draft.password && (
                      <div className="mt-1.5 h-1.5 rounded-full bg-[#232323] overflow-hidden">
                ...
                {draft.role === 'supervisor' && (
                  <FormField label="المكاتب المسموح بها" hint="اتركه فارغاً للسماح بالكل">
                    <div className="bg-[#0d0d0d] border border-[#232323] rounded-md p-2 max-h-32 overflow-y-auto grid grid-cols-2 gap-1 text-xs">
                      {offices.map(o=>(
                ...
                <div className="flex gap-2 pt-2">
                  ...
                  {editing && (
                    <button onClick={()=>toggleActive(editing)} className="px-4 py-2.5 rounded-lg bg-[#232323] hover:bg-[#2c2c2c] text-slate-300 text-sm">
...
function TrackingModal({ user, loc, stale, onClose }: { user: Profile; loc: any; stale: boolean; onClose: ()=>void }) {
  return (
    <div className="fixed inset-0 z-[600] flex items-center justify-center p-4" dir="rtl">
      ...
      <div className="relative w-full max-w-md bg-[#0d0d0d] border border-amber-500/30 rounded-2xl shadow-2xl overflow-hidden">
          ...
          {!loc ? <div className="text-center text-sm text-slate-500 py-6">لا توجد بيانات موقع</div> : (
            <>
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div className="bg-[#1a1a1a] border border-[#232323] rounded p-3"><div className="text-slate-500">خط العرض</div><div className="font-mono">{loc.lat.toFixed(5)}</div></div>
                <div className="bg-[#1a1a1a] border border-[#232323] rounded p-3"><div className="text-slate-500">خط الطول</div><div className="font-mono">{loc.lng.toFixed(5)}</div></div>
                <div className="bg-[#1a1a1a] border border-[#232323] rounded p-3"><div className="text-slate-500">الدقة</div><div>±{Math.round(loc.accuracyMeters)} م</div></div>
                <div className="bg-[#1a1a1a] border border-[#232323] rounded p-3"><div className="text-slate-500">آخر تحديث</div><div>{relativeTime(loc.updatedAt)}</div></div>
              </div>
